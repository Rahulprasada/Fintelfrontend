...
export const AuthContext = createContext<AuthContextType>({
  user: null,
  login: async () => {
    console.error("!!! CRITICAL ERROR: The real 'login' function was not called.");
  },
  register: async () => {
    console.error("!!! CRITICAL ERROR: The real 'register' function was not called.");
  },
  logout: () => {},
  loading: true,
...
export const AuthProvider = ({ children }: AuthProviderProps) => {
  ...
  useEffect(() => {
    ...
    if (token) {
      axios
        .get(`${API_URL}user-status/`, { headers: { Authorization: `Bearer ${token}` } })
        .then((response) => {
  ...
  return (
    <AuthContext.Provider value={{ user, login, register, logout, loading, isEmailConfirmed }}>
      {children}
